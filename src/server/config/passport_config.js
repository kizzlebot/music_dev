...
/**
 * Soundcloud Passport.js authentication verify.
 * User must be logged in to be able to add soundcloud to their account.  If user already has token,
 * then a token isn't generated
 * @param  {[type]} req          [description]
 * @param  {[type]} accessToken  [description]
 * @param  {[type]} refreshToken [description]
 * @param  {[type]} profile      [description]
 * @param  {[type]} done){                    if (req.user){    User.findById(req.user.id, (err, user) [description]
  ...
  if (req.user){
    User.findById(req.user.id, (err, user) => {
      ...
      if (user.soundcloud && _.find(req.user.tokens, {type:'soundcloud'})) {
        req.flash('errors', {msg:'already linked'})
...
passport.use(new LastFmStrategy({
  ...
  // If user logged in
  if (!req.user){
    req.flash('error', {msg: 'User must be logged in'});
